# This script is borrowed and modified from https://github.com/HongwenZhang/PyMAF/models/pymaf_net.py
...
from core.path_config import MASKS
...
class Regressor(nn.Module):
    ...
    def forward(self, x, init_pose=None, init_shape=None, init_cam=None, n_iter=1, J_regressor=None):
        ...
        pred_rotmat = rot6d_to_rotmat(pred_pose).view(batch_size, 24, 3, 3) # 6D seamless rotation representation
...
class PyMAFSAM(nn.Module):

    def __init__(self, masks = MASKS, smpl_mean_params=SMPL_MEAN_PARAMS, pretrained=True):
    # def __init__(self, smpl_mean_params=SMPL_MEAN_PARAMS, pretrained=True):
        ...
        ma_feat_len = self.maf_extractor[-1].Dmap.shape[0] * cfg.MODEL.PyMAF.MLP_DIM[-1] # MLP_DIM: [256, 128, 64, 5]
        ...
        masks_points = np.load(masks)
        self.register_buffer('masks_points', torch.tensor(masks_points))
        grid_len = 441
        grid_feat_len = grid_len * cfg.MODEL.PyMAF.MLP_DIM[-1]
    ...
    def forward(self, x, J_regressor=None):
        ...
        for rf_i in range(cfg.MODEL.PyMAF.N_ITER):
            ...
            self.maf_extractor[rf_i].im_feat = s_feat_i # maf for mash alignment feature
            self.maf_extractor[rf_i].cam = pred_cam

            if rf_i == 0:
                sample_points = self.masks_points.narrow(0, random.randint(0,(self.masks_points.shape[0])-batch_size), batch_size) # [960,441,2]->[32,441,2]
                # sample_points = torch.transpose(self.points_grid.expand(batch_size, -1, -1), 1, 2) # 441 = 21*21
...
def pymafsam_net(masks_points, smpl_mean_params, pretrained=True):
# def pymaf_net(smpl_mean_params, pretrained=True):
    """ Constructs an PyMAF model with ResNet50 backbone.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = PyMAFSAM(masks_points, smpl_mean_params, pretrained)
