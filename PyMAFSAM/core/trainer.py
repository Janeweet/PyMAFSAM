...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        ...
        if self.options.regressor == 'hmr':
            ...
            self.smpl_female = SMPL(model_path=path_config.SMPL_MODEL_DIR,
                                    gender='female',
                                    create_transl=False).to(self.device)
            self.model = pymaf_net(path_config.MASKS, path_config.SMPL_MEAN_PARAMS, pretrained=True)
    ...
    def train(self, epoch):
        ...
        if self.options.rank == 0:
            # prints a dynamically updating progressbar every time a value is requested
    ...
    def train_step(self, input_batch):
        ...
        gt_betas = input_batch['betas'] # SMPL beta parameters(shape)
    ...
    def fit(self):
        ...
        for epoch in tqdm(range(self.epoch_count, self.options.num_epochs), total=self.options.num_epochs, initial=self.epoch_count):
            ...
            if cfg.MODEL.PyMAF.N_ITER == 3 and self.options.rank == 0:
                performance = self.evaluate()
