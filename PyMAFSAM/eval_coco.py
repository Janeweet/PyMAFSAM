...
from models import pymaf_net, SMPL, pymafsam_net
...
parser.add_argument('--regressor', type=str, choices=['pymaf_net', 'pymafsam_net'], default='pymaf_net', help='Name of the SMPL regressor.')
...
def run_evaluation(model, dataset_name, dataset, result_file,
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
            ...
            if options.regressor == 'pymaf_net':
            ...
            elif options.regressor == 'pymafsam_net':
                preds_dict, _ = model(images)
                pred_rotmat = preds_dict['smpl_out'][-1]['rotmat'].contiguous().view(-1, 24, 3, 3)
                pred_betas = preds_dict['smpl_out'][-1]['theta'][:, 3:13].contiguous()
                pred_camera = preds_dict['smpl_out'][-1]['theta'][:, :3].contiguous()
...
if __name__ == '__main__':
    ...
    if args.regressor == 'pymafsam_net':
        model = pymaf_net(path_config.MASKS, path_config.SMPL_MEAN_PARAMS, pretrained=True)
